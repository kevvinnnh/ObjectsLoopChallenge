...
// Each recipe should be in its own div, that can be recCont. 
//We are declaring it here since we will want to change its value with each itteration of the loop.
...
for (let i = 0; i < recipesList.length; i++) {
    ...
    let ingredients = recipesList[i].ingredients.map(ingredient => {
        return ingredient;
    }).join('<br />');
    recipeCont.innerHTML = `
    <div class="allRecipes">
    <div class="title">${recipesList[i].title}</div>
    <div class="information">
    <b><u>Time to cook</u> : </b>${recipesList[i].time}
    <b><u>Calories</u> : </b> ${recipesList[i].calories}
    <b><u>Ingredients</u></b>
    ${ingredients} </div>
    </div>`;

    recipes.appendChild(recipeCont);
} 